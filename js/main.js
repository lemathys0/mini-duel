...
import {
    ...
    setMatchVariables,
    timerMax,
    timerInterval, // <-- AJOUTE CET IMPORT
...
function startTimer(startTime) {
    ...
    setTimerInterval(clearInterval(timerInterval)); // <-- CORRECTION ICI
    ...
    setTimerInterval(setInterval(() => {
        ...
        if (typeof startTime !== 'number' || isNaN(startTime)) {
            ...
            setTimerInterval(clearInterval(timerInterval)); // <-- CORRECTION ICI AUSSI
...
async function handleForfeit() {
    ...
    try {
        ...
        if (onDisconnectRef) { // Annule l'opération onDisconnect si elle a été configurée
            onDisconnectRef.cancel().catch(err => console.error("Error cancelling old onDisconnect:", err));
        }
